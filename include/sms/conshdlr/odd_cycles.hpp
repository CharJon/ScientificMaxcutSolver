#ifndef SMS_ODD_CYCLES_HPP
#define SMS_ODD_CYCLES_HPP
...
#include "conshdlr_extended.hpp"

#include "sms/auxiliary/odd_cycle.hpp"
#include "sms/auxiliary/limited_dijkstra.hpp"

using NetworKit::node;
using NetworKit::Edge;
using NetworKit::Graph;

const std::string NO_SAVE_STRING = "NONE";

/** C++ constraint handler for triangles (on edge variables) */
class ConshdlrOddCycles : public ObjConshdlrExtended {
public:
    std::string saveDir;

    /** constructor */
    ConshdlrOddCycles(
            McExtendedModel *p_model,
            SCIP_Bool delay = TRUE,
            std::string p_saveDir = NO_SAVE_STRING
    )
            : ObjConshdlrExtended(p_model->scip, "Odd Cycles", "odd cycles constraints separator",
                                  2147483640, 2147483640, 2147483640, 1, 1, 1, 100,
                                  delay, FALSE, TRUE, SCIP_PROPTIMING_BEFORELP, SCIP_PRESOLTIMING_FAST),
              saveDir(std::move(p_saveDir)), model(p_model) {
...
std::vector<OddCycle> getOddCycles(const Graph &, const Graph *);

std::vector<node> getPathBiDir(const Graph &, ulong, ulong, double);

std::vector<node> getPathDijkLim(const Graph &, ulong, ulong, double);

std::vector<node> getPathDijk(const Graph &, ulong, ulong, double);

Graph createGsGraph(const Graph *, SCIP_SOL *, McExtendedModel *, std::string save_file = "NONE");

bool shouldSave(std::string);

/** creates and captures a TSP subtour constraint */
SCIP_RETCODE SCIPcreateConsOddCycles(
